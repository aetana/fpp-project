...
public class MyHashTable implements Iterable {
	...
	public void put(Object key, Object value) {
		if (key == null)
			return;
		if (this.loadFactor() >= MyHashTable.DEFAULT_LOAD_FACTOR)
			this.rehash();
	...
	private void rehash() {
		LinkedList<Entry>[] tableTemp = new LinkedList[this.tableSize * 2];
		...
		while (it.hasNext()) {
			...
			int hashcode = key.hashCode();
			int hash = (int) Math.abs(hashcode % (this.tableSize * 2));

			if (tableTemp[hash] == null) {
				tableTemp[hash] = new LinkedList<Entry>();
			}
